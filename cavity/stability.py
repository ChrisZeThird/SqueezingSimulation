...
def plot_stability_contour(param_x, param_y, x_range, y_range, fixed_params, discrete_R=None, discrete_l_crystal=None, discrete_L=None):
    ...
    contour = plt.pcolormesh(X_grid, Y_grid, stability_map, shading='auto', cmap='viridis')
    plt.colorbar(contour)

    if discrete_R is not None:
        for R_val in discrete_R:
            plt.axvline(x=R_val, color='r', linestyle='--', label=f'R = {R_val} mm')

    if discrete_l_crystal is not None:
        for lc_val in discrete_l_crystal:
            plt.axhline(y=lc_val, color='b', linestyle='--', label=f'l_crystal = {lc_val} mm')

    if discrete_L is not None:
        for L_val in discrete_L:
            plt.axhline(y=L_val, color='g', linestyle='--', label=f'L = {L_val} mm')
    ...
    plt.gca().text(0.95, 0.95, fixed_params_text, transform=plt.gca().transAxes,
